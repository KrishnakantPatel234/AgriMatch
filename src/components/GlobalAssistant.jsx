...
const GlobalAssistant = () => {
  ...
  const knowledgeBase = {
    ...
    prices: {
      en: {
        vegetables: [
          { crop: 'Tomato', price: '₹25-32/kg', trend: '↗️ Rising', market: 'Pune APMC' },
          { crop: 'Potato', price: '₹18-22/kg', trend: '➡️ Stable', market: 'Delhi Azadpur' },
          { crop: 'Onion', price: '₹30-38/kg', trend: '↗️ Rising', market: 'Lasalgaon' },
          { crop: 'Cabbage', price: '₹12-15/kg', trend: '➡️ Stable', market: 'Bengaluru' },
          { crop: 'Cauliflower', price: '₹20-25/kg', trend: '↘️ Falling', market: 'Kolkata' }
        ],
        fruits: [
          { crop: 'Mango (Alphonso)', price: '₹400-600/kg', trend: '↗️ Seasonal', market: 'Ratnagiri' },
          { crop: 'Banana', price: '₹25-35/kg', trend: '➡️ Stable', market: 'Tamil Nadu' },
          { crop: 'Grapes', price: '₹50-70/kg', trend: '↗️ Rising', market: 'Nashik' }
        ]
      },
      hi: {
        vegetables: [
          { crop: 'टमाटर', price: '₹25-32/किग्रा', trend: '↗️ बढ़ रहा', market: 'पुणे APMC' },
          { crop: 'आलू', price: '₹18-22/किग्रा', trend: '➡️ स्थिर', market: 'दिल्ली आजादपुर' },
          { crop: 'प्याज', price: '₹30-38/किग्रा', trend: '↗️ बढ़ रहा', market: 'लसलगांव' }
        ],
        fruits: [
          { crop: 'आम (अल्फोंसो)', price: '₹400-600/किग्रा', trend: '↗️ मौसमी', market: 'रत्नागिरि' }
        ]
      },
      mr: {
        vegetables: [
          { crop: 'टोमॅटो', price: '₹25-32/किग्रा', trend: '↗️ वाढत आहे', market: 'पुणे APMC' },
          { crop: 'बटाटा', price: '₹18-22/किग्रा', trend: '➡️ स्थिर', market: 'दिल्ली आजादपुर' },
          { crop: 'कांदा', price: '₹30-38/किग्रा', trend: '↗️ वाढत आहे', market: 'लसलगांव' }
        ],
        fruits: [
          { crop: 'आंबा (अल्फोंसो)', price: '₹400-600/किग्रा', trend: '↗️ हंगामी', market: 'रत्नागिरि' }
        ]
      }
    },
    ...
    weather: {
      en: {
        ...
        advisory: 'Good for vegetable growth. Consider harvesting before thunderstorms.'
      },
      hi: {
  ...
  const respond = async (userMessage) => {
    ...
    // Price queries
    if (lower.includes('price') || lower.includes('कीमत') || lower.includes('किंमत') || lower.includes('rate')) {
      const priceList = langData.prices?.vegetables?.map(item => 
    ...
    // Weather queries
    if (lower.includes('weather') || lower.includes('मौसम') || lower.includes('हवामान') || lower.includes('rain')) {
    ...
    if (lower.includes('pest') || lower.includes('कीट') || lower.includes('किडी') || lower.includes('disease') || lower.includes('रोग')) {
      const pestInfo = langData.pests?.common?.map(pest => 
    ...
    if (lower.includes('cold storage') || lower.includes('storage') || lower.includes('कोल्ड') || lower.includes('स्टोरेज') || lower.includes('साठवण')) {
    ...
    // Transport queries
    if (lower.includes('transport') || lower.includes('ट्रांसपोर्ट') || lower.includes('वाहतूक') || lower.includes('delivery') || lower.includes('मार्ग')) {
    ...
    // Buyer queries
    if (lower.includes('buyer') || lower.includes('खरीददार') || lower.includes('खरेदीदार') || lower.includes('seller') || lower.includes('vendor')) {
    ...
    if (user?.role === 'transport' && (lower.includes('route') || lower.includes('return') || lower.includes('मार्ग'))) {
      return language === 'hi'
    ...
    if (user?.role === 'coldstorage' && (lower.includes('capacity') || lower.includes('कैपेसिटी') || lower.includes('क्षमता'))) {
      return language === 'hi'
  ...
  const send = async (e) => {
    ...
    setMsg('');
    setChat(prev => [...prev, { type: 'user', message: userMessage, timestamp: new Date() }]);

    // Try real AI service first
    try {
      setIsTyping(true);
      const { aiAPI } = await import('../services/Api');
      const res = await aiAPI.chat({ message: userMessage, language, role: user?.role });
      const reply = res.data?.reply;
      if (reply) {
        setIsTyping(false);
        setChat(prev => [...prev, { type: 'ai', message: reply, timestamp: new Date() }]);
        return;
      }
    } catch (err) {
    ...
    setIsTyping(false);
    setChat(prev => [...prev, { type: 'ai', message: response, timestamp: new Date() }]);
  };
      ...
      {open && (
        <div className="w-96 bg-white rounded-xl shadow-2xl border border-gray-200 overflow-hidden animate-fade-in">
          <div className="bg-gradient-to-r from-green-600 to-green-700 text-white px-4 py-3 flex justify-between items-center">
            ...
            <div className="flex items-center gap-2">
              <button onClick={clearChat} className="p-1 hover:bg-green-500 rounded transition-colors" title="Clear chat">
                🗑️
              </button>
              <button onClick={() => setOpen(false)} className="p-1 hover:bg-green-500 rounded transition-colors">
                ✕
              </button>
            </div>
          </div>
            ...
            {chat.map((m, i) => (
              <div key={i} className={`flex ${m.type === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] rounded-2xl px-4 py-3 ${
                  ...
                  <div className={`text-xs mt-1 ${m.type === 'user' ? 'text-green-100' : 'text-gray-500'}`}>
